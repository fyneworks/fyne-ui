...
export const calculateTotal = items => {
  console.log('fyne/ui: calculateTotal', {items});
  let grand_total = 0;
  items.forEach(item=>{
    console.log('fyne/ui: calculateTotal LOOP item', {item});
    
    let line_total = lineTotal(item);
    grand_total += line_total;

    console.log('fyne/ui: calculateTotal LOOP item DONE', {line_total,grand_total});
  })
  console.log('fyne/ui: calculateTotal DONE', {grand_total});
  return n(parseFloat(grand_total).toFixed(2));
}
export const lineTotal = item => {
  console.log('fyne/ui: calculateTotal lineTotal item', {item});
  ...
  console.log('fyne/ui: calculateTotal lineTotal item DONE', {item,line_quant,line_magni,line_price,line_subto,line_surch,line_total,line_final});
